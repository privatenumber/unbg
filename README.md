...
Use `--crop` to trim every fully transparent edge while preserving the subject's natural aspect ratio. Use `--crop 0.02` to ignore pixels with alpha at or below `0.02` only when finding the bounds. The crop threshold does not change pixels inside the resulting rectangle.
...
```ts
import { cropTransparent, detectBackground, differenceMatting } from 'unbg/core'
```
